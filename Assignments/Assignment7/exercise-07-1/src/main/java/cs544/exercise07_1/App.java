...
//    }
    ...
    static {
		...
		sessionFactory = HibernateUtils.getSessionFactory(Arrays.asList(Airline.class, 
				                                                        Flight.class, 
				                                                        Airport.class,
				                                                        Airplane.class				                                                        
				                                                        ));
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // TODO update HQL
            @SuppressWarnings("unchecked")// select * from Flight f join airplane  where capacity>500 and f.origin_id in (select id from airport where country='USA');
           List<Flight> flights = session.createQuery("from Flight f where f.airplane.capacity>500 and f.origin.country='USA'").list();
        ...
        try {
            ...
            // TODO update HQL
            @SuppressWarnings("unchecked")//select distinct arpl from airplane arpl join Flight f  on f.airplane_id=airplane.id join airline on f.airline_id=airline.id where airplane.model='A380';
            List<Airline> airlines = session.createQuery("Select distinct arl from Flight f join f.airline arl join f.airplane arpl where arpl.model = 'A380'").list();
        ...
        try {
            ...
            // TODO update HQL
            @SuppressWarnings("unchecked")//select * from Flight f join airplane on f.airplane_id=airplane.id join airline on f.airline_id=airline.id where airplane.model='747' and airline.name <> 'Star Alliance';
        ...
        try {
            ...
            // TODO update HQL
            @SuppressWarnings("unchecked")//select * from Flight f where departureDate<'2009-08-07' and departureTime<'12:00:00';
            List<Flight> flights = session.createQuery("from Flight f where f.departureDate<'2009-08-07' and f.departureTime<'12:00:00'").list();
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:");
            for (Flight flight : flights) {
                System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s\n",
